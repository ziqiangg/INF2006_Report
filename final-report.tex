...
On identical Dev/Val splits, enhanced v5.3 reduces total runtime from 4.2 min to 1.1 min on Dev ($4.0\times$) and from 1.5 min to 40.1 s on Val ($2.2\times$). In the baseline, the dominant costs are analytics lookup build, simulation initialisation, and the simulation loop; in the enhanced pipeline, costs shift to one-off FinBERT precompute and vectorised analytics, while lookup and loop phases are reduced via additive caches and bisect-based retrieval.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{./figures/fig_base_enhanced_profiling.png}
\caption{CPU phase-level profiling of baseline vs. enhanced v5.3 on Dev/Val splits. Enhanced v5.3 achieves lower total runtime despite additional NLP and multi-indicator logic, supporting the practical impact of OPT-1 through OPT-9.}
...
\begin{itemize}
    ...
    \item \textbf{MACD (12/26/9):} MACD histogram $< 0$ contributes one bearish vote in the same exit check.
...
In implementation, the effective order target also applies a minimum notional floor:
\[
\text{effective\_target} = \max\!\left(100,\; \left\lfloor\text{base\_target} \times \text{path\_mult} \times \text{vol\_mult} \times \text{dd\_brake}\right\rfloor\right)
\]
...
\begin{enumerate}[(1)]
    ...
    \item \textbf{Bearish 2-of-3}: Exits when 2-of-3 bearish conditions hold: RSI $< 35$ (below \texttt{RSI\_SELL}), MACD histogram $< 0$, or price $<$ MA-50. C6 modifies this for Path C-low entries: RSI contributes no bearish vote during the first 4 weeks after entry (grace period), then reactivates. For positions entered with a negative FinBERT sentiment score (C9), the exit threshold is reduced from 2-of-3 to 1-of-3, so any single bearish technical signal triggers exit. This is the primary exit mechanism, accounting for 91.5--92.8\% of all exits. The sell signal set is deliberately asymmetric with the buy signal set: volume ratio is absent from sell signals (volume surges confirm entries but their absence does not signal deterioration), and MA-50 replaces MA-200 (its shorter lookback detects trend breaks faster than the regime filter).
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Final performance summary: baseline vs. enhanced v5.3.}
...
\caption{Enhanced v5.3 portfolio value trajectory on the Dev split (2000--2017). 
...
trajectory on the Dev split, achieving a total return of 247.65\% from initial capital 
of \$100,000. The portfolio demonstrates disciplined drawdown management with maximum 
peak-to-trough decline limited to -22.10\%, substantially below the baseline's -35.42\%.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{./figures/v5.3_val-portfolio.png}
\caption{Enhanced v5.3 portfolio value trajectory on the Val split (2018--2024). 
...
validation Sharpe of 1.84 compared to 0.41 baseline, with maximum drawdown of only 
-12.49\% (vs.\ -35.42\% baseline).

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{./figures/v5.3_baselineVSenhanced.png}
\caption{Side-by-side performance comparison: baseline (red) vs.\ enhanced v5.3 (blue) 
...
total return increases by $5.1\times$ (63.05\% vs.\ 12.45\%), Sharpe ratio rises from 
0.41 to 1.84 (exceeding the $1.0$ ``good'' threshold), maximum drawdown improves by 
22.93 percentage points, win rate gains 14.16 points (41.91\% vs.\ 27.75\%), and 
portfolio volatility drops from 43.80\% to 19.40\%. Figure~\ref{fig:baseline-vs-enhanced} 
