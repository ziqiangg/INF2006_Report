...
%
% FIX: Original used two \begin{minipage}[t]{0.5\textwidth} blocks separated
% by ~ (a non-breaking inter-word space).  The combined width was therefore
% \textwidth + ~space, causing a 6.93pt overfull \hbox at titlepage lines 37-59.
% Fix: reduce each minipage to 0.48\textwidth and replace ~ with \hfill so the
% two panels fill exactly \textwidth with a flexible gap between them.
%----------------------------------------------------------------------------------------
