...
Aggregate statistics are near-identical: annualised return 19.92\% (Dev) vs.\ 17.00\% (Val), annualised volatility 38.01\% vs.\ 37.62\%, raw Sharpe 0.524 vs.\ 0.452. However, the Val split contains episodes without Dev precedent: the COVID-19 crash (S\&P $-34\%$ in 33 days), the fastest Fed rate-hike cycle since 1980, and the SVB banking crisis. The enhanced strategy's path distribution across these regimes (Val: all-three 16.6\%, path-A 58.8\%, path-B 23.5\%) is structurally identical to Dev (18.9\%, 58.4\%, 21.7\%), confirming the signal architecture captures a persistent cross-market phenomenon.
...
\textbf{Base grid: SIZING\_SCALE and BRAKE\_HWM\_WEEKS.} A 6-combination grid over \texttt{SIZING\_SCALE} $\in \{1.10, 1.20, 1.30\}$ and \texttt{BRAKE\_HWM\_WEEKS} $\in \{26, 52\}$ was exhaustively evaluated across all 15 anchored IS/OOS windows. Trailing-stop parameters were held fixed at \texttt{TRAIL\_ACTIVATE\_PCT}$=0.20$, \texttt{TRAIL\_PCT}$=0.12$, and \texttt{MAX\_HOLD\_WEEKS\_PATH\_B}$=10$ (selection values from prior hyperparameter tuning). The selection converged decisively to \texttt{SIZING\_SCALE} $= 1.10$ (13 of 15 IS windows won by this value) and \texttt{BRAKE\_HWM\_WEEKS} $= 26$ (14 of 15 windows selected this value). Walk-Forward Efficiency was 1.467, with 73\% of OOS windows achieving Sharpe $> 1.0$, 80\% achieving positive returns, and 100\% maintaining maximum drawdowns better than $-30\%$, demonstrating strong out-of-sample stability across regime transitions and crash periods (2008--2009, COVID-19).
