...
Shows cumulative growth reaching \$316k from \$100k initial capital, with volatility 
scaling and drawdown management visible during crisis periods.}
\label{fig:v5.3-portfolio-dev}
\end{figure}
Figure~\ref{fig:v5.3-portfolio-dev} illustrates the enhanced strategy's compounding 
trajectory on the Dev split, reaching \$316,452 from initial capital of \$100,000 
(247.65\% return). The portfolio demonstrates disciplined drawdown management with 
the maximum peak-to-trough decline limited to -22.10\%, substantially below the 
baseline's -35.42\%.
...
Performance over 7 years of out-of-sample data including COVID-19 crash, Fed rate hikes, 
and SVB banking crisis. Demonstrates strategy robustness across unprecedented regimes.}
...
on Val split. Enhanced strategy achieves superior Sharpe ratio (1.84 vs. 0.41), 
reduced drawdown (-12.49\% vs. -35.42\%), and 5.1× higher total returns.}
\label{fig:baseline-vs-enhanced}
\end{figure}
Figure~\ref{fig:baseline-vs-enhanced} visualizes the performance gap across all five 
metrics simultaneously, demonstrating the superiority of the five-gate architecture 
with path-dependent sizing and continuous volatility scaling.
